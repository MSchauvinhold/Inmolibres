...
export default async function DashboardPage() {
  ...
  if (isParticular) {
    ...
    const [totalPropiedades, totalClientes, visitasSemana, consultasNoLeidas, visitasProximas] =
      await Promise.all([
        Promise.resolve(propiedades.length),
        db.cliente.count({ where: { agenteId: userId, inmobiliariaId: null } }),
        db.visita.count({ where: { agenteId: userId, createdAt: { gte: inicioSemana } } }),
        propiedadIds.length
          ? db.consulta.count({ where: { propiedadId: { in: propiedadIds }, leida: false } })
          : Promise.resolve(0),
        db.visita.findMany({
          where: { agenteId: userId, estado: "PENDIENTE", fechaHora: { gte: hoy } },
    ...
    const stats = [
      { label: "Mis propiedades", value: totalPropiedades, sub: "publicadas", icon: Building2, color: "text-brand-primary", bg: "bg-brand-primary/10", href: "/propiedades" },
      { label: "Clientes", value: totalClientes, sub: "registrados", icon: Users, color: "text-info", bg: "bg-info/10", href: "/clientes" },
      ...
      { label: "Consultas sin leer", value: consultasNoLeidas, sub: "del marketplace", icon: AlertCircle, color: "text-warning", bg: "bg-warning/10", href: "/consultas" },
    ];
  ...
  const [
    ...
    db.propiedad.count({ where: { inmobiliariaId: inmobiliariaId! } }),
    db.propiedad.count({ where: { inmobiliariaId: inmobiliariaId!, estado: "DISPONIBLE", publicada: true } }),
    db.cliente.count({ where: { inmobiliariaId: inmobiliariaId! } }),
    db.visita.count({ where: { inmobiliariaId: inmobiliariaId!, createdAt: { gte: inicioSemana } } }),
    db.contratoAlquiler.count({ where: { inmobiliariaId: inmobiliariaId!, fechaFin: { gte: hoy } } }),
    db.contratoAlquiler.count({ where: { inmobiliariaId: inmobiliariaId!, fechaFin: { gte: hoy }, estadoPago: "ATRASADO" } }),
    db.consulta.count({ where: { inmobiliariaId: inmobiliariaId!, leida: false } }),
    db.visita.findMany({
      where: { inmobiliariaId: inmobiliariaId!, estado: "PENDIENTE", fechaHora: { gte: hoy } },
      orderBy: { fechaHora: "asc" },
      take: 5,
      include: {
        propiedad: { select: { titulo: true } },
    ...
    db.contratoAlquiler.findMany({
      where: { inmobiliariaId: inmobiliariaId!, fechaFin: { gte: hoy, lte: en30dias } },
